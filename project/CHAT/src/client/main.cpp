...
/*
    之前的设计是用户登录成功后开启读线程，最开始只有主线程，既负责发又负责收，当第一次登录时没有问题，
    但当用户登录后执行loginout后再次登录时，主线程再rec上阻塞，且此时子线程已经开启了，所以主线程和子线程都会卡在rec函数上，所以需要让负责读的子线程一开始就启动
...
sem_t rwsem;
//记录用户的登陆状态
//需要采用原子类型，哪怕读写线程肯定不会同时访问共享变量，但是如果不用原子类型，可能写线程再做if判断时，用的是自己的缓存，而不是内存中读线程已经更新过的值，
//所以如果不用原子类型，这个时候要加上volatile修饰，保证每次从内存中读取新值，但这也表明volatile并不能保证原子性，
//每次线程都重新读取新值，假设主线程先读取新值，然后切到子线程读取新值，然后两线程都++，结果还是出错
//volatile只能保证每次读取新值，但无法保证读取值---值++------写入值整体是一个原子操作
